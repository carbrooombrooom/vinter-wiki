...
<ParamField query="-QueryPort" type="integer" default="27016">
  Sets the port used for server queries (e.g., server browser listings).

  **Example:** `-QueryPort=27016`
</ParamField>

You can use these parameters when starting your server. For example:

```shell
LongvinterServer.exe -GamePort=7777 -QueryPort=27016
```
